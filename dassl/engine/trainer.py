...
class SimpleTrainer(TrainerBase):
    ...
    def test(self, split=None):
        ...
        if self.cfg.TEST.TTA == True:
            print("Test Time Augmentation is on:")
            for batch_idx, batch in enumerate(tqdm(data_loader)):
                input, label = self.parse_batch_test(batch)
                imgs = []
                #print("input", input.shape) 100 3 224 224
                for img in input:
                # print("img", img.shape) 3 224 224
                    imgs.append(T.ToPILImage()(img))
                pred = []
                for tfm in self.tfms:
                    batch_img = []
                    for img in imgs:
                        tfm_input = tfm(img)
                        batch_img.append(tfm_input)
                    batch_img = torch.stack(batch_img).cuda()
                #  print("batch_img", batch_img.shape) # 100 3 224 224
                    output = self.model_inference(batch_img)
                    pred.append(output)
                predictions = torch.stack(tuple(pred))   
                #print("predictions", predictions.shape) # 5 100 10
                pred_agg = torch.mean(predictions, dim=0)
            # print("label", label.shape) 
                self.evaluator.process(pred_agg, label)
        else:
            for batch_idx, batch in enumerate(tqdm(data_loader)):
                input, label = self.parse_batch_test(batch)
